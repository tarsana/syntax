<?php namespace Tarsana\Syntax\Internals;

function split(string $text, string $separator, string $surrounders = '""[](){}', string $wrappers = '""'): array {
 ...
 * ('(Hey)', '""()') => 'Hey'
 */
function unwrap(string $text, string $wrappers): string {
  $size = strlen($wrappers);
  for ($i = 0; $i < $size; $i += 2) {
    if (substr($text, 0, 1) == substr($wrappers, $i, 1) && substr($text, -1) == substr($wrappers, $i + 1, 1)) {
...
function join(array $items, string $separator): string {
  return implode(
    $separator,
    array_map(function (string $item) use ($separator) {
