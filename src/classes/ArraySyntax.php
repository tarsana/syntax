...
class ArraySyntax extends Syntax {
  ...
  public function parse(string $text): array {
    $index = 0;
    $items = Internals\split($text, $this->separator);
    $array = [];
    try {
      ...
      throw new ParseException($this, $text, $index + $e->position(), "Unable to parse the item '{$item}'", $extra, $e);
    }
  ...
  public function dump($values): string {
    ...
    return Internals\join($items, $this->separator);
  }
}
